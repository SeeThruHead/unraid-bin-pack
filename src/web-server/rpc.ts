...
const createPatternNode = (name: string) => ({
  pattern: `/${name}`,
  name,
  children: [] as string[],
})
...
const mergePatternChildren = (patterns: Array<{ pattern: string; name: string; children: string[] }>) => {
  ...
  return Array.from(grouped.entries())
    .map(([pattern, nodes]) => ({
      ...
      name: nodes[0]?.name || '',
      children: Array.from(
        new Set(nodes.flatMap(n => n.children))
      ).sort(),
    }))
    .sort(sortByName)
}
...
const scanPatternsForPaths = (diskPaths: string[]) =>
  Effect.gen(function* () {
    ...
    const allPatterns = yield* Effect.all(
      diskPaths.map(path => scanDiskForPatterns(fs, path))
    )
...
const rpc = new Hono()
  .get('/disks', async (c) =>
    c.json(await Effect.runPromise(discoverDisks))
  )
  ...
  .get('/apply-stream', async (c) => {
    ...
    return streamSSE(c, async (stream) => {
      await Effect.runPromise(
        Effect.gen(function* () {
          yield* Effect.promise(() => writeSSEMessage(stream, 'start', { message: 'Starting plan execution...' }))
            ...
            try {
              ...
              if (stats.size > lastPosition) {
                ...
                for (const line of lines) {
                  await writeSSEMessage(stream, 'progress', { message: line })
                }
          ...
          try {
            ...
            if (stats.size > lastPosition) {
              ...
              const buffer = Buffer.alloc(stats.size - lastPosition)
              yield* Effect.tryPromise(() => fileHandle.read(buffer, 0, buffer.length, lastPosition))
            ...
            // Ignore errors reading final log content
          }

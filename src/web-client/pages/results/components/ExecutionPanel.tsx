...
export function ExecutionPanel({ diskProjections, selectedDiskPaths, onVerify, onExecutionComplete }: ExecutionPanelProps) {
  ...
  const { events, status, error, result, execute, reset } = usePlanExecution({
    ...
    diskProjections,
    selectedDiskPaths,
    onComplete: useCallback((executionResult: ExecutionResult, dryRun: boolean) => {
      if (dryRun) {
    ...
    onVerified: useCallback((diskSpace: DiskResponse[]) => {
      onVerify(diskSpace)
    }, [onVerify])
  })
  ...
  return (
    <>
      ...
      {events.length > 0 && hasCompleted && (
        <ExecutionProgress events={events} executing={false} />
      )}
      {isExecuting && !hasCompleted && (
        <ExecutionProgress events={events} executing={true} />
      )}
    </>
  )
}

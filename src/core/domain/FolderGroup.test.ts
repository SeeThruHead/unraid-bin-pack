...
describe("FolderGroup", () => {
  describe("groupByImmediateFolder", () => {
    ...
    test("root-level files go to empty folder", () => {
      const files: FileEntry[] = [makeFile("readme.txt", 1000), makeFile("config.json", 500)];
    ...
    describe("keepTogether heuristics", () => {
      test("small folders are kept together", () => {
        const files: FileEntry[] = [
          makeFile("small-project/a.txt", 100_000_000),
        ...
        expect(groups).toHaveLength(1);
        expect(groups[0]?.keepTogether).toBe(true);
      });

      test("folders with dominant file are kept together", () => {
        const files: FileEntry[] = [
          makeFile("movie/main.mkv", 45_000_000_000),
          makeFile("movie/subs.srt", 5_000_000_000)
        ];
      ...
      test("large folders without dominant file can be split", () => {
        const files: FileEntry[] = [
          makeFile("anime/season1/ep01.mkv", 500_000_000),
        ...
        expect(groups).toHaveLength(1);
        expect(groups[0]?.totalBytes).toBe(2_000_000_000);
        expect(groups[0]?.largestFileBytes).toBe(500_000_000);
  ...
  describe("groupByTopLevelFolder (deprecated)", () => {
    ...
    test("handles root-level files (no folder)", () => {
      const files: FileEntry[] = [makeFile("readme.txt", 10), makeFile("notes.md", 20)];
    ...
    test("handles mixed root and folder files", () => {
      const files: FileEntry[] = [makeFile("readme.txt", 10), makeFile("docs/guide.md", 20)];

...
export const consolidateSimple = (
  ...
  Effect.gen(function* () {
    ...
    let rankedDisks: ReturnType<typeof rankDisksByFullness>

    if (options.srcDiskPaths && options.srcDiskPaths.length > 0) {
      const srcDiskPaths = options.srcDiskPaths
      rankedDisks = filteredWorldView.disks
        .filter((d) => d.path !== "/mnt/disks" && srcDiskPaths.includes(d.path))
        .map((disk) => {
          const diskFiles = filteredWorldView.files.filter((f) => f.diskPath === disk.path)
          const usedBytes = diskFiles.reduce((sum, f) => sum + f.sizeBytes, 0)
          const usedPct = (usedBytes / disk.totalBytes) * 100
          return { path: disk.path, usedPct, freeBytes: disk.freeBytes }
        })
      yield* Effect.logDebug(
        `Using ${rankedDisks.length} explicitly selected source disk(s): ${srcDiskPaths.join(", ")}`
      )
    } else {
      rankedDisks = rankDisksByFullness(filteredWorldView.disks, filteredWorldView.files)
      rankedDisks = rankedDisks.filter((d) => d.path !== "/mnt/disks")
      yield* Effect.logDebug(
        `Ranked ${rankedDisks.length} disks by fullness for iterative consolidation`
      )
    }

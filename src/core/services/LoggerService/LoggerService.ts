...
export interface LoggerService {
  readonly plan: {
    ...
    readonly existingPlanWarning: (stats: {
      completed: number;
      failed: number;
      pending: number;
    }) => Effect.Effect<void>;
    ...
    readonly planStats: (stats: {
      movesPlanned?: number;
      skipped?: number;
      totalBytes: number;
      totalMoves?: number;
      disksEvacuated?: number;
      foldersPlaced?: number;
      foldersExploded?: number;
    }) => Effect.Effect<void>;
  ...
  readonly apply: {
    ...
    readonly insufficientSpace: (
      moves: ReadonlyArray<{ disk: string; needed: number; available: number }>
    ) => Effect.Effect<void>;
    readonly diskStatsChanged: Effect.Effect<void>;
    readonly diskStatsChangedWarning: (
      changes: ReadonlyArray<{ disk: string; before: number; after: number }>
    ) => Effect.Effect<void>;
  ...
  readonly show: {
    ...
    readonly targetDiskHeader: (
      disk: string,
      count: number,
      totalBytes: number
    ) => Effect.Effect<void>;
    readonly moveEntry: (entry: {
...
export const LoggerServiceLive = Layer.succeed(LoggerServiceTag, {
  plan: {
    ...
    planStats: (stats) =>
      Effect.gen(function* () {
        yield* Console.log(`\n📊 Plan Summary:`);
        if (stats.movesPlanned !== undefined)
          yield* Console.log(`   Moves planned: ${stats.movesPlanned}`);
        if (stats.skipped !== undefined) yield* Console.log(`   Skipped: ${stats.skipped}`);
        if (stats.totalMoves !== undefined)
          yield* Console.log(`   Total moves: ${stats.totalMoves}`);
        yield* Console.log(`   Total data: ${formatSize(stats.totalBytes)}`);
        if (stats.disksEvacuated !== undefined)
          yield* Console.log(`   Disks to evacuate: ${stats.disksEvacuated}`);
      }),
  ...
  apply: {
    ...
    allSourceFilesExist: () => Console.log("✓ All source files exist"),

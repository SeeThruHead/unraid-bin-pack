...
const makePlan = (moves: FileMove[]): MovePlan => ({
  ...
  summary: {
    ...
    totalBytes: moves
      ...
      .reduce((acc, m) => acc + m.file.sizeBytes, 0),
...
describe("RsyncTransferService", () => {
  describe("executeAll", () => {
    test("dry-run returns preview without executing", async () => {
      const execState = { called: false };
      const StubShell = Layer.succeed(ShellServiceTag, {
        exec: (_command) => {
          execState.called = true;
      ...
      expect(execState.called).toBe(false);
    ...
    test("handles failed transfers", async () => {
      const StubShell = Layer.succeed(ShellServiceTag, {
        exec: (_command) => Effect.succeed({ stdout: "", stderr: "permission denied", exitCode: 1 })
      });

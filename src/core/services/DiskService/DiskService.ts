  ...
  Effect.gen(function* () {
    ...
    const validateDiskPath = (path: string): Effect.Effect<void, DiskError> =>
      pipe(
        ...
        Effect.catchAll((e) => {
          if (
            e instanceof DiskNotFound ||
            e instanceof DiskNotADirectory ||
            e instanceof DiskNotAMountPoint ||
            e instanceof DiskPermissionDenied ||
    ...
    return {
      ...
      discover: (paths: string[]) => Effect.all(paths.map(getStats), { concurrency: "unbounded" }),

...
export const runPlan = (options: PlanOptions, isInteractive: boolean = false) =>
  Effect.gen(function* () {
    ...
    const finalOptions = yield* isInteractive
      ? Effect.gen(function* () {
          ...
          return yield* interactivePlanPrompts(discoveredDisks);
        })
    ...
    const parsed = yield* parsePlanOptions(finalOptions);
    ...
    if (result.stats.movesPlanned === 0) {

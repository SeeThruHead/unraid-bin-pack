...
export const interactivePlanPrompts = (
  ...
  Effect.gen(function* () {
    ...
    for (const disk of discoveredDisks) {
      ...
      yield* Console.log(
        `   ${disk.path}: ${usedGB}/${totalGB} GB used (${usedPct}%), ${freeGB} GB free`
      );
    }
    yield* Console.log("");
    ...
    const selectedDirs = yield* selectDirectoriesEffect(discoveredDisks.map((d) => d.path)).pipe(
      Effect.orDie
    );
    ...
    const planFile = yield* Prompt.text({
      ...
      Effect.map((s) => (s.trim() === "" || s.trim() === "/config/plan.sh" ? undefined : s.trim()))
    );
